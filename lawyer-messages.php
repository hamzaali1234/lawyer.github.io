<?php
$titleTag = '';
$title = "Lawyers Website - Lawyers Messages";// for dynamic title on selected page
$currentPage = 'Lawyers Messages';// for add dynamic active class on selected page on header
$pageTitle = "Lawyers Messages";// for add dynamic text of Page Title Section
$page="lawyer-messages.php";// for add dynamic url text of Page Title Section
    ...
    <div class="row">
        ...
        <div class="blogWraper">
          <div class="comments margin-top30">
            ...
            <?php
                ...
                if($search_input == "") {
                    if($page=="all"){
                      $sql = "select * from reply inner join user on reply.lawyer_id=user.id where reply.client_id='".$id."' order by reply.id desc";
                    } else {
                      $sql = "select * from reply inner join user on reply.lawyer_id=user.id where reply.client_id='".$id."' order by reply.id desc limit ".$page1.",10";
                    }
                } else {
                    $sql = "select * from reply inner join user on reply.lawyer_id=user.id where reply.client_id='".$id."' and reply.lawyer_name like '%$search_input%' order by reply.id desc";
                }
                $result = $conn->query($sql);
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $timestamp = strtotime($row['lawyer_reply_on']);
                        $date = date('F j, Y, g:ia',$timestamp);
            ?>
              <!-- Client Message -->
              <li class="media">
                <div class="media-left"> <a href="#."> <img class="media-object img-responsive" src="<?php echo $row["image"]; ?>" alt=""> <br>
                  </a> </div>
                <div class="media-body">
                  <h6 class="media-heading"><?php echo $row["lawyer_name"]; ?><span> <?php echo $date; ?></span></h6>
                  <p><?php echo $row["lawyer_message"]; ?></p>
                </div>
              </li>
            <?php } } ?>
            </ul>
            ...
            <ul class="pagination">
              <?php
                if($page>1){
                    $last = $page-1;
                    echo '<li class="page-item"><a class="page-link hvr-bob" href="lawyer-messages.php?page='.$last.'">Previous</a></li>';
                } else {
                    echo '<li class="page-item"><a class="page-link" style="background:grey;pointer-events: none;" href="#">Previous</a></li>';
                }
                $query2=mysqli_query($conn,"select * from reply inner join user on reply.lawyer_id=user.id where reply.client_id='".$id."' order by reply.id desc");
                $count=mysqli_num_rows($query2);
                $a=$count/10;
                $a=ceil($a);
                for($b=1;$b<=$a;$b++){
              ?>
                <li class="page-item"><a class="page-link hvr-bob <?php if($b==$page){echo 'active';} ?>" style="color:white;" href="lawyer-messages.php?page=<?php echo $b; ?>"><?php echo $b; ?></a></li> 
              <?php } 
                if($b>2){
                  if($page>$page1){
                      $next = $page+1;
                      echo '<li class="page-item"><a class="page-link hvr-bob" href="lawyer-messages.php?page='.$next.'">Next</a></li>';
                  } else {
                      echo '<li class="page-item"><a class="page-link" style="background:grey;pointer-events: none;" href="#">Next</a></li>';
                  }
                ?>
                <li class="page-item"><a class="page-link hvr-bob <?php if($page=='all'){echo 'active';} ?>" href="lawyer-messages.php?page=all">View All</a></li>
                <?php
                }else{
                  echo '<li class="page-item"><a class="page-link" style="background:grey;pointer-events: none;" href="#">Next</a></li>';
      ...
      <?php
        ...
        <script type="text/javascript">
          ...
          if(search_input != ""){
            document.getElementById("searching_error_msg").innerHTML = 'You are looking for someone with the message <span style="color:#db3636;font-weight:bold;">"<?php echo $search_input; ?>"</span>, messages from this lawyer were not found.';
          }else{
            document.getElementById("searching_error_msg").innerHTML = 'Lawyers messages are not available';
          }
